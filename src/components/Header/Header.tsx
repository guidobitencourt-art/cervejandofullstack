...
const Header = () => {
  ...
  return (
    <header className="bg-[#FFF5D4] flex justify-between items-center p-8 relative">
      ...
      <button
        className="flex md:hidden flex-col gap-[5px] bg-transparent border-0 p-0 cursor-pointer z-10"
        aria-label="Abrir menú"
        aria-expanded={open}
        onClick={() => setOpen(v => !v)}
      >
        <span className="block w-[30px] h-[3px] rounded-[5px] bg-gray-800 transition-all" />
        <span className="block w-[30px] h-[3px] rounded-[5px] bg-gray-800 transition-all" />
        <span className="block w-[30px] h-[3px] rounded-[5px] bg-gray-800 transition-all" />
      </button>
